...
def run(input_file: Path, output_dir: Path, config: dict) -> Path:
    ...
    Select the first eligible numeric predictor and evaluate on a stratified
    holdout. Refit preprocessing and the model on all labelled rows before
    saving the model and evaluation metrics.

    :param input_file: Processed training CSV containing predictors and the
                       target.
    :param output_dir: Directory in which to save the model and metrics.
    :param config: Training settings including target, join key, split size and
                   seed.
    ...
    # Provide an actionable failure when the processing node has not produced an
    # input
    ...
    # Neither the transaction identifier nor the label is a valid baseline
    # predictor.
    ...
    # Use one predictor deliberately to keep the template example quick and
    # transparent
    feature = features[0]
    ...
    # Ensure the configured fraction leaves data available on both sides of the
    # split
    ...
    # Fit imputation and scaling inside the pipeline using only the training
    # split.
    model = make_pipeline(
    ...
    # Retain holdout probabilities for metrics before refitting the final
    # estimator
    ...
    # Keep holdout probabilities for metrics, then use all labelled rows for
    # final scoring.
    model.fit(x, y)
