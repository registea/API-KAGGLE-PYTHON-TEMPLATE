...
def resolve_metadata(
    ...
    Apply command-line overrides and environment identity before the JSON
    defaults. Return a copy so preparing a run does not alter the template
    metadata.
    ...
    """
    # Copy the source mapping so command-line overrides do not edit the template
    # file
    result = dict(metadata)

    # Resolve a full ID, or compose it from environment username and slug.
    # Prefer a complete command-line or environment identity when one is
    # available
    kernel_id = options.kernel_id or settings.kernel_id
    # Compose the identity only when both individual environment values are
    # present
    ...
    if kernel_id:
        result["id"] = kernel_id
        # Kaggle prioritises numeric id_no over id; avoid targeting a stale
        # kernel.
    ...
    # Numeric Kaggle identifiers can silently override the intended owner and
    # slug
